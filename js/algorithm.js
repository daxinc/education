...
algorithmModule.controller("AlgorithmController", ["$scope", "$log", function($scope, $log) {
    var _generateAdditions = function() {
        var currentTime = new Date().getTime();
        var e = [];
        for (var i=300;i<400;i++) for (var j=200;j<300;j++) for (var k=100;k<200;k++) {
            e.push([i, j, k]);
        }
        $log.debug("Generated " + e.length + " equations.");
        e = _suffle(e);
        e = e.slice(0, 12);
        $log.debug("Get random " + e.length + " additions in " + (new Date().getTime() - currentTime) + "ms.");
    ...
    var _generateSubtractions = function() {
        var currentTime = new Date().getTime();
        var e = [];
        for (var i=500;i<600;i++) for (var j=100;j<200;j++) for (var k=200;k<=300;k++) {
            if (i-j-k<0){
                $log.error("Algorithm error in generating subtraction: " + i + " - " + j + " - " + k + " = " + (i-j-k));
                continue;
            }
            e.push([i, j, k]);
        }
        ...
        e = _suffle(e);
        e = e.slice(0, 12);
        $log.debug("Get random " + e.length + " subtractions in " + (new Date().getTime() - currentTime) + "ms.");
